...
def NKA (graf):
    ...
    curr = ['']
    for i in range (len(graf)):
        if graf[i][0] != curr[0]:       # если это новая буква (такой еще нет в нка)
            numb += 1                   # то создаем новую строчку
            curr[0] = graf[i][0]        # и называем ее этой буквой
            if graf[i][2] == '0':       # если переход по 0, добавляем состояние куда переходим в первый столбик текущей строки
                line = [curr[:], [graf[i][1]], [], curr[0] in ends, curr[0] in begins]
            elif graf[i][2] == '1':     # если переход по 1, добавляем состояние куда переходим во второй столбик текущей строки
                line = [curr[:], [], [graf[i][1]], curr[0] in ends, curr[0] in begins]
            nka.append(line)
        else:                           # если такая строка уже есть, 
            if graf[i][2] == '0':           
                nka[numb][1].append(graf[i][1])     #то дописываем переходы в нужный столбик (0 или 1)
            elif graf[i][2] == '1':            
                nka[numb][2].append(graf[i][1])
    for lett in ends:                               #  отдельно для конечных состояний
        isInNka = False                             # (поскольку из них может ничего не выходить)
        for j in range(len(nka)):                   
            if nka[j][0][0] == lett:
                isInNka = True
        if isInNka == False:                        # находим подобное состояние и тоже добавляем его в НКА
            nka.append([[lett], [], [], lett in ends, lett in begins])
    
    i = 0                                             # сортируем таблицу для красоты
...
def DKA(nka):
    begins.sort()
    dka = [[begins, [], [], False, False]]          # инициализируем первую строку ДКА множеством нач. состояний
    newPositions = [dka[0][0]]                      # в NewPositions храним все уже созданные состояния ДКА
    
    for lett in begins:                             # для того чтобы заполнить первую строку, идем по нач. состояниям
        for i in range (len(nka)):
            if nka[i][0][0] == lett:                # в НКА находим состояние с это буквой
                for l in range (len(nka[i][1])):    # и добавляем ее переходы в таблицу, если их в этой строчке еще нет
    ...
    dka[0][1].sort()            # сортируем полученные переходы чтобы не былы двух разных состояний типа АВ и ВА
    dka[0][2].sort()
    i = 0
    while i < len(dka):                 # идем по всем имеющимся строкам ДКА 
        for j in range (1,3):           # и по их переходам по 0 и 1
            if dka[i][j] not in newPositions:       # если еще нет строки с там переходом,
                ...
                line = [dka[i][j], [], [], False, False]    # и так же, как я для первого состояния (описано выше),
                for lett in (dka[i][j]):                    # заполняем его переходы
...
# добавить цикличность по убирания эквивалентных состояний